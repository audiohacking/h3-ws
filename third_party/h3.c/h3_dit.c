...
/* Load an INT8 weight tensor. */
static h3_gpu_tensor *i8_2(h3_dit *dit, const char *name, uint64_t rows,
                           uint64_t columns, char *error, size_t error_size) {
    uint64_t shape[] = {rows, columns};
    return h3_weight_load_int8(dit->weights, dit->gpu, name, 2, shape,
                               error, error_size);
}

/* Load scale tensor for an INT8 weight (per-output-channel F32 scale). */
static h3_gpu_tensor *scale_1(h3_dit *dit, const char *name, uint64_t rows,
                              char *error, size_t error_size) {
    uint64_t shape[] = {rows, 1};
    return h3_weight_load_f32(dit->weights, dit->gpu, name, 2, shape,
                              error, error_size);
}

/* Check if a weight exists as INT8 (for pre-quantized model detection). */
static int has_int8_weight(h3_dit *dit, const char *prefix, const char *suffix) {
    char name[160];
    snprintf(name, sizeof(name), "%s%s", prefix, suffix);
    return h3_weight_exists_int8(dit->weights, name);
}
...
#define LOAD2_BF16(field, suffix, rows, columns) do {                          \
    snprintf(name, sizeof(name), "%s%s", prefix, suffix);                    \
    block->field = bf2(dit, name, rows, columns, error, error_size);            \
    if (!block->field) return 0;                                                \
} while (0)
#define LOAD2_INT8(field_int8, field_scales, suffix, rows, columns) do {       \
    snprintf(name, sizeof(name), "%s%s", prefix, suffix);                    \
    block->field_int8 = i8_2(dit, name, rows, columns, error, error_size);      \
    if (!block->field_int8) return 0;                                           \
    snprintf(name, sizeof(name), "%s%s_scale", prefix, suffix);              \
    block->field_scales = scale_1(dit, name, rows, error, error_size);          \
    if (!block->field_scales) return 0;                                         \
} while (0)
    /* Norms are always BF16. */
    ...
    /* Check if weights are pre-quantized INT8 (Comfy format). */
    int preloaded_int8 = has_int8_weight(dit, prefix, "attn.qkv_proj.weight");
    if (preloaded_int8) {
        /* Load INT8 weights + scales directly; skip BF16 path. */
        LOAD2_INT8(qkv_int8, qkv_scales, "attn.qkv_proj.weight", INNER * 3, HIDDEN);
        LOAD2_INT8(out_int8, out_scales, "attn.out_proj.weight", HIDDEN, INNER);
        LOAD2_INT8(fc1_int8, fc1_scales, "mlp.fc1.weight", FFN * 2, HIDDEN);
        LOAD2_INT8(fc2_int8, fc2_scales, "mlp.fc2.weight", HIDDEN, FFN);
    } else {
        /* Standard BF16 load (will be quantized at runtime if int8_* enabled). */
        LOAD2_BF16(qkv, "attn.qkv_proj.weight", INNER * 3, HIDDEN);
        LOAD2_BF16(out, "attn.out_proj.weight", HIDDEN, INNER);
        LOAD2_BF16(fc1, "mlp.fc1.weight", FFN * 2, HIDDEN);
        LOAD2_BF16(fc2, "mlp.fc2.weight", HIDDEN, FFN);
    }
#undef LOAD1
#undef LOAD2_BF16
#undef LOAD2_INT8
...
static int load_core(h3_dit *dit, h3_dit_progress progress, void *opaque,
    ...
    for (unsigned index = 0; index < H3_DIT_BLOCKS; index++) {
        ...
        if (dit->ssd_streaming) {
            ...
            /* Skip runtime quantization if weights were pre-loaded as INT8. */
            h3_dit_block *block = &dit->blocks[index];
            if (dit->int8_mlp && !block->fc1_int8 &&
                !quantize_block_mlp(dit, block, error, error_size)) return 0;
            if (dit->int8_qkv && !block->qkv_int8 &&
                !quantize_block_qkv(dit, block, error, error_size)) return 0;
            if (dit->int8_attention_out && !block->out_int8 &&
                !quantize_block_attention_out(dit, block, error, error_size))
                return 0;
        }
        report(progress, opaque, "load transformer core", (int)index + 1,
               H3_DIT_BLOCKS);
...
static h3_dit *load_dit(const char *weight_directory,
    ...
    /* Detect pre-quantized INT8 weights (e.g., Comfy-Org format). */
    int preloaded_int8 = h3_weight_exists_int8(dit->weights,
                                               "blocks.0.attn.qkv_proj.weight");
    dit->gpu = h3_gpu_create(shader_source_path, error, error_size);
    if (!dit->gpu) goto failed;
    if (preloaded_int8) {
        if (!h3_gpu_has_int8_mlp(dit->gpu)) {
            fail(error, error_size,
                 "pre-quantized INT8 weights require Metal GPU with INT8 support");
            goto failed;
        }
        if (dit->ssd_streaming) {
            fail(error, error_size,
                 "pre-quantized INT8 weights are not compatible with SSD streaming");
            goto failed;
        }
    }
    dit->nax_mlp = dit->fused_mlp && h3_gpu_has_nax_mlp(dit->gpu);
    /* Force INT8 compute path when weights are pre-loaded as INT8. */
    dit->int8_mlp = preloaded_int8 ||
                    (!dit->ssd_streaming && dit->fused_mlp &&
                     !use_slower_bf16_mlp &&
                     h3_gpu_has_int8_mlp(dit->gpu));
    dit->int8_qkv = preloaded_int8 ||
                    (!dit->ssd_streaming && !use_slower_bf16_qkv &&
                     dit->sequence >= 128 &&
                     h3_gpu_has_int8_mlp(dit->gpu));
    dit->int8_attention_out = preloaded_int8 ||
                              (!dit->ssd_streaming &&
