...
/* Latent-only dump for TAEH3-style previews: no full VAE residency. */
typedef struct {
    const char *dir;
    int latent_t;
    int latent_h;
...
#define H3_LATENT_MAGIC "H3L1"

static int h3_dump_denoise_latent(int completed_steps, int total_steps,
                                  const float *video_latent,
                                  size_t video_elements, void *opaque) {
    h3_latent_dump *dump = opaque;
    if (!dump || !dump->dir || !video_latent) return 1;
    size_t expected = (size_t)24 * (size_t)dump->latent_t *
                      (size_t)dump->latent_h * (size_t)dump->latent_w;
    if (video_elements != expected) return 1;

    char path[1024];
    snprintf(path, sizeof(path), "%s/step_%04d.bin", dump->dir, completed_steps);
    FILE *fp = fopen(path, "wb");
    if (!fp) return 0; /* best-effort: never abort denoise for a preview dump */
    /* Header: magic[4] + u32 version,step,total,C,T,H,W (little-endian host). */
    uint32_t hdr[7];
    hdr[0] = 1; /* version */
    hdr[1] = (uint32_t)completed_steps;
    hdr[2] = (uint32_t)total_steps;
    hdr[3] = 24;
    hdr[4] = (uint32_t)dump->latent_t;
    hdr[5] = (uint32_t)dump->latent_h;
    hdr[6] = (uint32_t)dump->latent_w;
    if (fwrite(H3_LATENT_MAGIC, 1, 4, fp) != 4 ||
        fwrite(hdr, sizeof(hdr[0]), 7, fp) != 7 ||
        fwrite(video_latent, sizeof(float), video_elements, fp) != video_elements) {
        fclose(fp);
        return 0;
    }
    fclose(fp);

    /* latest.json is what the Python watcher polls. */
    snprintf(path, sizeof(path), "%s/latest.json", dump->dir);
    fp = fopen(path, "wb");
    if (fp) {
        fprintf(fp,
                "{\"step\":%d,\"total\":%d,\"channels\":24,"
                "\"t\":%d,\"h\":%d,\"w\":%d,\"file\":\"step_%04d.bin\"}\n",
                completed_steps, total_steps,
                dump->latent_t, dump->latent_h, dump->latent_w,
                completed_steps);
        fclose(fp);
    }
    fprintf(stderr, "h3: latent preview %d/%d dumped (%dx%dx%d)\n",
            completed_steps, total_steps,
            dump->latent_t, dump->latent_h, dump->latent_w);
    return 0;
}
...
h3_result *h3_generate(h3_ctx *ctx, const char *prompt,
    ...
    if (progress.cancelled) goto cleanup;
    h3_latent_dump latent_dump = {0};
    int use_latent_dump = params->preview_latent_dir &&
                          params->preview_latent_dir[0];
    int use_show_vae = params->preview_denoise && !use_latent_dump;
    if (use_latent_dump && params->preview_denoise) {
        fprintf(stderr,
                "h3: --preview-latent set; skipping full --show VAE residency\n");
    }
    if (use_show_vae) {
        ...
        if (progress.cancelled) goto cleanup;
    } else if (use_latent_dump) {
        if (mkdir(params->preview_latent_dir, 0755) != 0 && errno != EEXIST) {
            h3_set_error(ctx, "cannot create preview latent dir %s: %s",
                         params->preview_latent_dir, strerror(errno));
            goto cleanup;
        }
        latent_dump.dir = params->preview_latent_dir;
        latent_dump.latent_t = temporal.video_t;
        latent_dump.latent_h = latent_h;
        latent_dump.latent_w = latent_w;
        fprintf(stderr, "h3: latent preview dump -> %s\n",
                params->preview_latent_dir);
    }
    ...
    h3_rng_fill_normal(&audio_rng, audio, audio_count);
    {
        ...
        if (preview_decoder) {
            preview_cb = h3_deliver_denoise_preview;
            preview_opaque = &live_preview;
        } else if (use_latent_dump) {
            preview_cb = h3_dump_denoise_latent;
            preview_opaque = &latent_dump;
        }
