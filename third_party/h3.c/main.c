...
static void usage(const char *program) {
    fprintf(stderr,
        ...
        "      --preview-latent DIR  Dump denoise latents for TAEH3 preview (no VAE)\n"
...
int main(int argc, char **argv) {
    ...
    static const struct option options[] = {
        ...
        {"preview-latent", required_argument, NULL, OPT_PREVIEW_LATENT},
    ...
    while ((option = getopt_long(argc, argv, "d:p:o:h", options, NULL)) != -1) {
        switch (option) {
            ...
            case OPT_PREVIEW_LATENT: params.preview_latent_dir = optarg; break;
