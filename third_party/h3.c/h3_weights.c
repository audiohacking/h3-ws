...
static h3_gpu_tensor *load_tensor(const h3_weight_store *store, h3_gpu *gpu,
    ...
    /* Allow F16 when BF16 is expected (GPU will convert). */
    int f16_to_bf16 = (dtype == H3_DTYPE_BF16 && tensor->dtype == H3_DTYPE_F16);
    if (!f16_to_bf16 && (tensor->dtype != dtype || tensor->ndim != ndim)) {
    ...
    if (f16_to_bf16) {
        /* Load F16 and convert to BF16 on the GPU. */
        h3_gpu_tensor *f16 = h3_gpu_tensor_load_f16(gpu, header->path,
                                                     tensor->file_offset,
                                                     (size_t)elements);
        if (!f16) {
            fail(error, error_size, "cannot load F16 %s: %s", name,
                 h3_gpu_error(gpu));
            return NULL;
        }
        result = h3_gpu_tensor_new_bf16(gpu, (size_t)elements);
        if (!result ||
            !h3_gpu_begin(gpu) ||
            !h3_gpu_cast_f16_to_bf16(gpu, result, f16, (uint32_t)elements) ||
            !h3_gpu_submit(gpu)) {
            fail(error, error_size, "cannot convert F16 to BF16 for %s: %s",
                 name, h3_gpu_error(gpu));
            h3_gpu_tensor_free(f16);
    ...
    if (dtype == H3_DTYPE_BF16 && ndim == 2 &&
        !h3_lora_fuse(gpu, result, name, shape[0], shape[1], error,
                      error_size)) {
        h3_gpu_tensor_free(result);
        return NULL;
    }
...
h3_gpu_tensor *h3_weight_load_int8(const h3_weight_store *store, h3_gpu *gpu,
                                   const char *name, int ndim,
                                   const uint64_t *shape,
                                   char *error, size_t error_size) {
...
int h3_weight_exists_int8(const h3_weight_store *store, const char *name) {
